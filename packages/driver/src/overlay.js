...
// All nodes carry data-browser-bridge-ui="1" so observations can exclude them. Fully decoupled from execution: the cursor never
...
(() => {
  if (window.__browserBridgeDriveOverlayLoaded) return;
  window.__browserBridgeDriveOverlayLoaded = true;

  const ROOT_ID = "browser-bridge-drive-overlay";
  const HOP_MS = 100;
  let root = null;
  let cursor = null;
  let masks = [];
  ...
  function el(tag, opts = {}) {
    ...
    node.setAttribute("data-browser-bridge-ui", "1");
  ...
  function ensureRoot() {
    ...
    // from CSS. aria-hidden — it is decorative chrome (data-browser-bridge-ui excluded).
    ...
    cursor.setAttribute("data-browser-bridge-ui", "1");
    const tag = el("div", { class: "nb-cursor-tag", text: "Bridge" });
  ...
  function begin(accent, ownerLabel = "Bridge") {
    ensureRoot();
    if (typeof accent === "string" && /^\s*\d+\s*,\s*\d+\s*,\s*\d+\s*$/.test(accent)) {
      root.style.setProperty("--nb-accent", accent);
    }
    const label = typeof ownerLabel === "string" && ownerLabel.trim() ? ownerLabel.trim().slice(0, 40) : "Bridge";
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    switch (message?.type) {
      case "BB_DRIVE_BEGIN": begin(message.accent, message.ownerLabel); sendResponse?.({ ok: true }); return false;
      case "BB_DRIVE_END": end(); sendResponse?.({ ok: true }); return false;
      case "BB_DRIVE_MOVE": enqueueHop(message.x, message.y); sendResponse?.({ ok: true }); return false;
      case "BB_DRIVE_CLICK": enqueueHop(message.x, message.y, () => ripple(message.x, message.y)); sendResponse?.({ ok: true }); return false;
      case "BB_DRIVE_FIELD": if (message.rect) fieldRing(message.rect); sendResponse?.({ ok: true }); return false;
      case "BB_DRIVE_SCROLL": sendResponse?.({ ok: true }); return false;
      case "BB_DRIVE_MASK": applyMasks(message.zones); sendResponse?.({ ok: true }); return false;
      case "BB_DRIVE_UNMASK": clearMasks(); sendResponse?.({ ok: true }); return false;
