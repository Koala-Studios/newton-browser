...
test("action schema fields survive parse and redact", () => {
  ...
  assert.throws(() => parseBrowserAction({ kind: "screenshot", sensitiveZones: [{ ref: "button-1" }] }), /exactly one bounded ref/u);
...
test("action values preserve exact whitespace and reject oversized payloads", () => {
  const value = "  exact value  ";
  assert.equal(parseBrowserAction({ kind: "fill", ref: "d1:e1", value }).value, value);
  assert.throws(() => parseBrowserAction({ kind: "fill", ref: "d1:e1", value: "x".repeat(65_537) }), /bounded/u);
  assert.throws(() => parseBrowserAction({
    kind: "fill_form",
    fields: [{ ref: "d1:e1", value: "x".repeat(65_537) }],
  }), /bounded/u);
});
...
test("click can atomically wait for its bounded causal effect", () => {
  ...
  assert.equal(ACTION_VARIANT_FIELDS.click.includes("waitFor"), true);
  assert.equal(ACTION_VARIANT_FIELDS.click.includes("timeoutMs"), true);
  assert.throws(
