...
export const BROWSER_COMPOSITE_REF_PATTERN_SOURCE = "^d[1-9][0-9]*(?::f[1-9][0-9]*)?:e[1-9][0-9]*$";
const REF_PATTERN = BROWSER_COMPOSITE_REF_PATTERN_SOURCE;
const VALUE_MAX_LENGTH = 65_536;
const targetFields = ["ref", "role", "name", "label", "placeholder", "testId", "exact", "text", "selector", "x", "y"] as const;
...
export const ACTION_VARIANT_FIELDS: Readonly<Record<BrowserActionKind, readonly string[]>> = Object.freeze({
  ...
  screenshot: ["sensitiveZones", "fullPage", "clip", "format", "quality"],
  click: [...targetFields, "waitFor", "timeoutMs"],
  fill: [...targetFields, "value"],
  type: [...targetFields, "value"],
...
const targetRequirement = {
  anyOf: [
    { required: ["ref"] }, { required: ["role"] }, { required: ["name"] },
...
const fieldSchemas: Record<string, unknown> = {
  ...
  exact: { type: "boolean" }, value: { type: "string", minLength: 1, maxLength: VALUE_MAX_LENGTH }, url: { type: "string", minLength: 1, maxLength: 500 },
  text: { type: "string", minLength: 1, maxLength: 240 }, selector: { type: "string", minLength: 1, maxLength: 240 },
  ...
  sensitiveZones: { type: "array", minItems: 1, maxItems: 32, items: { type: "object", additionalProperties: false, minProperties: 1, maxProperties: 1, oneOf: [{ required: ["ref"] }, { required: ["selector"] }, { required: ["name"] }, { required: ["label"] }], properties: { ref: { type: "string", pattern: REF_PATTERN }, selector: { type: "string", minLength: 1, maxLength: 240 }, name: { type: "string", minLength: 1, maxLength: 240 }, label: { type: "string", minLength: 1, maxLength: 240 } } } },
  fullPage: { type: "boolean" }, clip: { type: "object", additionalProperties: false, required: ["x", "y", "width", "height"], properties: { x: { type: "number" }, y: { type: "number" }, width: { type: "number", exclusiveMinimum: 0 }, height: { type: "number", exclusiveMinimum: 0 } } },
  ...
  fields: { type: "array", minItems: 1, maxItems: 32, items: { type: "object", additionalProperties: false, required: ["value"], ...targetRequirement, properties: { ref: { type: "string", pattern: REF_PATTERN }, role: { type: "string", maxLength: 80 }, name: { type: "string", maxLength: 240 }, label: { type: "string", maxLength: 240 }, placeholder: { type: "string", maxLength: 240 }, testId: { type: "string", maxLength: 240 }, selector: { type: "string", maxLength: 240 }, value: { type: "string", minLength: 1, maxLength: VALUE_MAX_LENGTH } } } },
  level: { enum: ["log", "info", "warn", "error", "debug"] }, pattern: { type: "string", maxLength: 240 },
  limit: { type: "integer", minimum: 1, maximum: 500 }, urlPattern: { type: "string", maxLength: 500 },
