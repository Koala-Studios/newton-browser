...
| Worker model loop | Since the audit: navigation-aware feedback and pending-navigation reads (D1), 1280×900 default and locale/timezone/viewport options (D2), direct discovery of deep action controls with main-content and dialog ranking (D3/D19), stable refs over eight snapshots (D5/D23), field-level argument errors (D6), `observe.query` (D7), no pattern redaction in model output (D22), start budget (D26). See the audit's "Fixed after the audit" table. |
| Embedding API | `apps/mcp-server/src/embedding.ts` (`dist/embedding.js`, replacing the private engine-candidate entry): `createBrowserEngine` gives a host process the model tool catalog and `call`, plus host-only live JPEG frames with latest-frame backpressure, `pause`/`resume`/`operatorInput` on the same page (model actions get `operator_control`), a per-session WebAuthn virtual authenticator with credential events kept off the model channel, and shared-login sign-in as a session (`beginSignIn`/`finishSignIn`). Live regressions `host-live-control` and `embedding-sign-in` cover frames, takeover, passkey enrollment and passkey sign-in in a fresh session, publish, and two concurrent signed-in sessions. |
