...
| Worker model loop | Since the audit: navigation-aware feedback and pending-navigation reads (D1), 1280×900 default and locale/timezone/viewport options (D2), direct discovery of deep action controls with main-content and dialog ranking (D3/D19), stable refs over eight snapshots (D5/D23), field-level argument errors (D6), `observe.query` (D7), no pattern redaction in model output (D22), start budget (D26), covering layers reported with their controls first (`observation.cover`, D4) and script-driven role-less clickables discoverable as `clickable` (D29). See the audit's "Fixed after the audit" table. |
