...
- `newton-browser-0.4.1.tgz`
- `newton-browser-extension-0.4.1.zip`
- `newton-browser-extension-0.4.1.zip.sha256`
...
```powershell
$expected = (Get-Content .\newton-browser-extension-0.4.1.zip.sha256).Split()[0]
$actual = (Get-FileHash .\newton-browser-extension-0.4.1.zip -Algorithm SHA256).Hash.ToLowerInvariant()
...
```bash
sha256sum --check newton-browser-extension-0.4.1.zip.sha256
```
