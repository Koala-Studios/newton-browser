...
const requestedClients = String(process.env.NEWTON_BROWSER_REAL_CLIENTS ?? "codex,claude").split(",").map((value) => value.trim().toLowerCase()).filter((value) => ["codex", "claude"].includes(value));
if (requestedClients.length === 0) throw new Error("NEWTON_BROWSER_REAL_CLIENTS must include codex or claude");
const tarball = path.resolve("artifacts/newton-browser-0.4.1.tgz");
const transcriptPath = path.resolve(`artifacts/real-client-transcripts-${browserTarget}-${requestedClients.join("-")}.json`);
const mcpConfigPath = path.join(fs.mkdtempSync(path.join(os.tmpdir(), "newton-browser-real-clients-")), "mcp.json");
const npxCommand = process.platform === "win32" ? "npx.cmd" : "npx";
const mcpServer = {
  command: npxCommand,
  args: ["--yes", "--package", tarball, "newton-browser"],
  env: { NEWTON_BROWSER_BROWSER: browserTarget, NEWTON_BROWSER_AUTH_MODE: "local_trust" },
};
fs.writeFileSync(mcpConfigPath, `${JSON.stringify({ mcpServers: { "newton-browser-qa": mcpServer } }, null, 2)}\n`);
const task = (label) => `Authorized deterministic Newton Browser release QA for ${label}. Use only the browser tools from newton-browser-qa. Call browser.status once, then ALWAYS start an owned_group session at exact origin ${fixture.origin}, even if status initially reports extension_disconnected. In that session: observe and extract the fixture-ready marker; fill Search records with exact value ${label.toLowerCase()}-needle; click Run fixture search and verify exact visible text search-result:${label.toLowerCase()}-needle; take a full-page screenshot with delivery=image and confirm you received visible non-empty image metadata/content; then finalize the session with close. Return exactly three lines: ${label}_CLIENT_OK, ${label}_SEARCH_RESULT=search-result:${label.toLowerCase()}-needle, ${label}_SCREENSHOT_OK. Do not use shell, filesystem, web search, or any other browser implementation.`;

try {
  const jobs = [];
  if (requestedClients.includes("codex")) jobs.push({ client: "codex", promise: run("codex", [
      "exec", "--ephemeral", "--sandbox", "read-only", "--skip-git-repo-check",
      "-c", `mcp_servers.newton-browser-qa.command=${JSON.stringify(mcpServer.command)}`,
      "-c", `mcp_servers.newton-browser-qa.args=${JSON.stringify(mcpServer.args)}`,
      "-c", `mcp_servers.newton-browser-qa.env.NEWTON_BROWSER_BROWSER=${JSON.stringify(browserTarget)}`,
      "-c", 'mcp_servers.newton-browser-qa.env.NEWTON_BROWSER_AUTH_MODE="local_trust"',
      "-c", 'mcp_servers.newton-browser-qa.default_tools_approval_mode="approve"',
  ...
  if (requestedClients.includes("claude")) jobs.push({ client: "claude", promise: run("claude", [
    ...
    "--allowedTools", [
      "mcp__newton-browser-qa__browser_status",
      "mcp__newton-browser-qa__browser_session_start",
      "mcp__newton-browser-qa__browser_observe",
      "mcp__newton-browser-qa__browser_act",
      "mcp__newton-browser-qa__browser_screenshot",
      "mcp__newton-browser-qa__browser_tabs_finalize",
    ].join(","),
    "--verbose",
...
function run(command, args) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => { child.kill(); reject(new Error(`${command} real-client task timed out`)); }, Number(process.env.NEWTON_BROWSER_REAL_CLIENT_TIMEOUT_MS ?? 300_000));
