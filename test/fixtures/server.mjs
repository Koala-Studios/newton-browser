...
export async function startFixtureServers({ port = 18231, crossOriginPort = port + 1 } = {}) {
  const main = http.createServer((request, response) => serve(request, response, root, { crossOrigin: `http://127.0.0.1:${crossOriginPort}` }));
  const cross = http.createServer((request, response) => serve(request, response, root, { crossOrigin: `http://127.0.0.1:${crossOriginPort}` }));
...
function serve(request, response, directory, options = {}) {
  ...
  if (request.url === "/redirect-cross") return response.writeHead(302, { location: `${options.crossOrigin}/cross-origin.html` }).end();
