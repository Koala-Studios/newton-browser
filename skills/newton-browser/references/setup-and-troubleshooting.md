...
operation. Run that exact entrypoint with `--version` first and require `0.6.4`.
...
| `configured_identity_recovery_unavailable` | Preserve the identity; Newton could not prove the prior exact browser ownership is gone. |
| `configured_identity_recovery_failed` | Preserve the identity and report the bounded lease recovery failure; never delete the lease manually. |
...
| `max_refs_exceeded` | On 0.6.4, make one fresh interactive observation; it starts a new bounded ref cycle without reload or navigation. On an older runtime, preserve the session and verify state with text/screenshot reads before upgrading. Never retry an uncertain effect. |
...
A popup or authentication tab is a session-owned page, not browser chrome. Newton 0.6.4
...
If CSS, fonts, icons, or login redirects fail, confirm the exact Newton 0.6.4 entrypoint
...
and confirm `browser.sessions.list` is empty. A future persistent-session start recovers
one stale lease automatically only when the recorded host, descendants, exact identity
root, and lock evidence all prove closure. The operator `lease-recover` command uses the
same proof and refuses ambiguous or live ownership.
